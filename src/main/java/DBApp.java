...
public class DBApp implements DBAppInterface {
  ...
  public void appendCsv(
    ...
    while (type.hasMoreElements()) {
      ...
      if (col.equals(cluster)) clus = true;
    ...
    for (int i = 0; i < tablesize; i++) {
      ...
      if (type == 0) {
        ...
        if (keyy < minn) {
          ...
          return i - 1;
        } else if (minn < keyy && keyy < maxx) return i;
      } else if (type == 2) {
        ...
        if (compmin < 0) {
          ...
          return i - 1;
        } else if (compmin > 0 && compmax < 0) return i;
      } else if (type == 3) {
  ...
  public static int getType(String name, String col) { // int 0 Double 1 String 2 Date 3
    ...
    try {
      ...
      br.close();
      if (s.equalsIgnoreCase("java.lang.Integer")) return 0;
      else if (s.equalsIgnoreCase("java.lang.Double")) return 1;
      else if (s.equalsIgnoreCase("java.lang.String")) return 2;
      else return 3;
  ...
  public static void deleteRecord(String record) {
    ...
    try {
      ...
      while ((line = br.readLine()) != null) // returns a Boolean value
      {
        ...
        min = row[1];
        max = row[2];
        if (!name.equalsIgnoreCase(record)) pw.println(name + "," + min + "," + max);
      }
  ...
  public void updateTable(
    ...
    while (keys.hasMoreElements()) {
      ...
      if (!flagover) p.get(pos[1]).set(j, Change);
      else {
  ...
  */
  ...
  public Vector<ArrayList<Object>> execEq(SQLTerm sqlTerm) throws DBAppException {
    ...
    if (cluster.equals(col)) {
      ...
      Page p = Page.deserialP(tname + pos[0]);
      if (!flagover) result.add(p.get(pos[1]));
      else {
  ...
  public static int binarysearchdouble(Page v1, Double key) {
    ...
    while (low <= high) {
      ...
      if (cond < 0) low = mid + 1;
      else if (cond == 0) return mid; // key found
      else high = mid - 1;
  ...
  public static int searchtableall(String tableName, Object key) throws DBAppException {
    ...
    for (int i = 0; i < tablesize; i++) {
      ...
      if (type == 0) {
        ...
        Integer maxx = Integer.valueOf(max);
        Integer keyy = (Integer) key;
        if (minn <= keyy && keyy <= maxx) return i;
      } else if (type == 2) {
        String keyy = (String) key;
        int compmin = keyy.compareTo(min);
        int compmax = keyy.compareTo(max);
        if (compmin >= 0 && compmax <= 0) return i;
      } else if (type == 3) {
        Date minn;
        try {
  ...
  public static int coloumnnum(String coloumn, String t1) {
    ...
    for (int i = 0; i < array.length; i++) {
      if (array[i].equals(coloumn)) return i;
    }
    return -1;
  }
